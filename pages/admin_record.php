        ...
        <div class="card shadow-sm mt-5">
            ...
            <div class="card-body">
                        ...
                        <div class="mb-3" style="display: inline-flex; justify-content: space-between; width: 100%;">
                            ...
                            <form method="GET" action="" style="flex: 1;">
                                <div class="mb-3"
                                    ...
                                    <!-- เลือกสถานะรวม -->
                                    <select id="combined_status" name="combined_status" class="form-select"
                                        style="width: 180px; font-size: 14px; font-weight: normal;"
                                        onchange="this.form.submit()">
                                        <option value="" selected disabled>กรุณาเลือกสถานะ</option>
                                        <option value="all" <?= (isset($_GET['combined_status']) && $_GET['combined_status'] === 'all') ? 'selected' : '' ?>>ทั้งหมด</option>
                                        ...
                                        <option value="hreturn_1" <?= (isset($_GET['combined_status']) && $_GET['combined_status'] === 'hreturn_1') ? 'selected' : '' ?>>สภาพปกติ
                                        </option>
                                        <option value="hreturn_3" <?= (isset($_GET['combined_status']) && $_GET['combined_status'] === 'hreturn_3') ? 'selected' : '' ?>>ผู้ยืมซ่อมแซม
                                        </option>
                                        <option value="hreturn_4" <?= (isset($_GET['combined_status']) && $_GET['combined_status'] === 'hreturn_4') ? 'selected' : '' ?>>ชดใช้เป็นพัสดุ
                                        </option>
                                        <option value="hreturn_7" <?= (isset($_GET['combined_status']) && $_GET['combined_status'] === 'hreturn_7') ? 'selected' : '' ?>>
                                            แนบไฟล์ค่าเสียหาย</option>
                                    </select>
                ...
                <div class="input-group mb-3" style="margin-top: -12px; margin-left: 1px; margin-right: 5px;">
                    <input type="text" id="searchEquipment" class="form-control" placeholder="ค้นหา" aria-label="Search"
                        aria-describedby="button-search" style="font-size: 14px; padding: 9px 12px;">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                if ($row['history_Status_BRS'] == 0) {
                                    ...
                                    echo "<span class='badge rounded-pill bg-secondary' style='font-size: 12px;'>  <i class='bi bi-check-circle'></i> รอคืน </span>";
                                ...
                                if ($row['hreturn_Status'] == 1) {
                                    echo "<span class='badge rounded-pill bg-success' style='font-size: 12px;'> <i class='bi bi-check-circle'></i> สภาพปกติ</span>";

                                } elseif ($row['hreturn_Status'] == 2) {
                                    echo "<span class='badge rounded-pill' style='font-size: 12px; background-color:hsl(14, 98.70%, 70.00%); color:rgb(59, 59, 59);'>  <i class='bi bi-exclamation-triangle'></i> สภาพไม่สมบูรณ์
                                 </span>";
                                ...
                                if ($row['hreturn_Status'] == 7) {
                                    echo "<div class='d-flex justify-content-center align-items-center gap-2' style='height: 100%;'> 
                                        ...
                                        <button onclick='changeStatusToZero(\"" . htmlspecialchars($row['device_Id']) . "\")' 
                                            class='btn btn-success rounded-pill px-3 py-1 text-white shadow-sm' style='font-size: 12px;'> 
                                ...
                                if (!empty($money_Image) && file_exists($imagePath)) {
                                    echo "<td>
                                            <a href='#' class='btn btn-secondary btn-sm' data-bs-toggle='modal' data-bs-target='#imageModal' data-bs-image='" . htmlspecialchars($imagePath) . "' title='ดูรูปภาพ'>
                                                <i class='bi bi-image-fill'></i> ดูรูปภาพ
                                            </a>
                                          </td>";
                                } else {
                                    if ($row['hreturn_Status'] == 7) {
                                        echo "<td style='background-color: #f8d7da; color: #721c24; font-weight: bold; text-align: center;'>
                                                ยังไม่แนบไฟล์
                                              </td>";
                                    } else {
                                        echo "<td style='text-align: center;'>
                                                <i class='bi bi-dash-circle' style='font-size: 1.5rem; color: #28a745;' title='ไม่ต้องการข้อมูล'></i>
                                              </td>";
                                    }
                                }
                    ...
                    function changeStatusToZero(deviceId) {
                        ...
                        Swal.fire({
                            ...
                            if (result.isConfirmed) {
                                // ผู้ใช้กดยืนยัน => ส่งคำขอ AJAX
                                const xhr = new XMLHttpRequest();
                                xhr.open("POST", "../connect/status/update.php", true);
                                xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");

                                // ส่ง deviceId ไปยังเซิร์ฟเวอร์
                                xhr.send("device_Id=" + encodeURIComponent(deviceId));

                                xhr.onload = function () {
                                    if (xhr.status === 200) {
                                        // เช็คว่าการอัปเดตสำเร็จหรือไม่
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <input type="hidden" id="device_Id" name="device_Id"
                                        value="<?php echo htmlspecialchars($_GET['device_Id'] ?? ''); ?>">

                                    <div class="form-group mb-4">
                                        <label for="money_Image">เลือกไฟล์รูปภาพ:</label>
                                        <input type="file" class="form-control" id="money_Image" name="money_Image"
